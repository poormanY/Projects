...
static CString StrTesterName[] = {"김정애", "양숙영", "송미순", "강순옥", "박상례", "차기선", "이현웅", "박순", "유상원"};
static CString strProgramType[] = { "일반", "최종검사1(PCB ASSY)", "최종검사2(ASSY)" };
// BOX 최대 유닛 개수
...
void MainDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_COMBO_PROGRAM_TYPE, m_ctrl_program_type);
}
...
//    oLogFont.lfHeight = -60;
	...
	for (int k = 0; k < 3; k++)		m_ctrl_program_type.InsertString(k, strProgramType[k]);
	m_ctrl_program_type.SetCurSel(0);
...
void MainDlg::OnSelchangeComboProgramType()
{
	// TODO: Add your control notification handler code here
	UpdateData(TRUE);
	int nComboProgramType;
	CString strProgramType;

	nComboProgramType = m_ctrl_program_type.GetCurSel();
	m_ctrl_program_type.GetLBText(nComboProgramType, strProgramType);
	m_COMBO_PROGRAM_TYPE = strProgramType;
