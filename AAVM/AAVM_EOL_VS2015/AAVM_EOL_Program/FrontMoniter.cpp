...
CString strFImagePath;

CFrontImg::CFrontImg(CWnd* pParent /*=NULL*/)
	: CDialog(CFrontImg::IDD, pParent)
{
...
void CFrontImg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CFrontImg, CDialog)
...
void CFrontImg::DrawImage(CDC *pDC)
{

	CImage Image;
	CClientDC dc(this);
	//	CPaintDC dc(this);
	HRESULT hresult = Image.Load(strFImagePath);
...
int CFrontImg::FrontDraw(CString Name, unsigned int StartNumber, unsigned int StopNumber)
{
	CImage Image;
	CClientDC dc(this);
   ...
   if(Number_FILE<9200)
   {
		...
		HRESULT hresult = Image.Load(strFImagePath);
		// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
		Image.TransparentBlt(dc.m_hDC, 0, 180, 1280, 720, RGB(255, 0, 0) );
	...
	if(Number_FILE<StopNumber)
   {
		...
		// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
		Image.TransparentBlt(dc.m_hDC, 0,	0, 1280, 720, RGB(255, 0, 0) );
		//Image.TransparentBlt(dc.m_hDC, 0,	0, 1280, 1024, RGB(255, 0, 0) );
